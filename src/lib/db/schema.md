...
The SQL implementation uses normalized tables. The types below are generic (Text, Integer, Timestamp/ISO String) but map to specific types in SQLite (`TEXT`, `INTEGER`) or PostgreSQL (`TEXT`, `TIMESTAMP`).
...
*Primary Key is Composite: (`user_id`, `headquarters_id`)*
