...
export class DynamoDBAdapter implements DatabaseAdapter {
  ...
  async getUserHeadquartersObjects(userId: string): Promise<Headquarters[]> {
    ...
    return items.map(this.mapToHeadquarters);
  }
  ...
  async getHeadquarters(): Promise<Headquarters[]> {
    ...
    return result.Items.map(this.mapToHeadquarters);
  }
  ...
  async getUserRequests(hqId: string, userId: string): Promise<AppRequest[]> {
    ...
    const command = new QueryCommand({
      TableName: TABLE_NAME,
      IndexName: "GSI1",
      KeyConditionExpression: "GSI1PK = :userPk AND begins_with(GSI1SK, :gsiSkPrefix)",
      ExpressionAttributeValues: {
